...
class GeneralManager {
    ...
    __Delete() {
        ...
        for key, value in this.hooks {
            nCheck := DllCall( "UnhookWinEvent", Uint,value )
        }
        DllCall( "GlobalFree", UInt, &this.HookProcAdr ) ; free up allocated memory for RegisterCallback
    ...
    _init(part) {
        ...
        If (part == "desktops") {
            ...
            ;   ;; listen for various shell events to trigger retiling
            ...
            ;   HookProcAdr := RegisterCallback("GeneralManager._onWindowEvent", "F")

            ;   ; ;; listen for minimize / restore window events
            ...
            ;   ; ;; listen for maximize window events
            ;   ; DllCall("CoInitialize", "uint", 0)
            ;   ; hWinEventHook := DllCall( "SetWinEventHook"
            ;   ;   , UInt,0x000B
            ;   ;   , UInt,0x000B
    ...
    _onDesktopChange(wParam, lParam, msg, winId) {
        ...
        ;; todo hide / raise floating windows
    ...
    _onWindowEvent( hWinEventHook, Event, hWnd, idObject, idChild, dwEventThread, dwmsEventTime ){
        Global cfg, logger

        SetFormat, Integer, Hex
        Event += 0, hWnd += 0, idObject += 0, idChild += 0
        ; winId := Format("0x{:x}", idObject)
        wnd := this.getWindow(hWnd)
        ; logger.debug("Window message received with message number '" . hWinEventHook . "' and window id '" . winId . "'.", "GeneralManager._onWindowEvent")

        if (Event = 0x0016) { ;; EVENT_SYSTEM_MINIMIZESTART
            ;MsgBox EVENT_SYSTEM_MINIMIZESTART
        }
        else if (Event = 0x0017) { ;; EVENT_SYSTEM_MINIMIZEEND
            ;MsgBox EVENT_SYSTEM_MINIMIZEEND
        }
        ; else if (hWinEventHook = 0x000B) { ;; EVENT_SYSTEM_MOVESIZEEND
        ;   MsgBox EVENT_SYSTEM_MOVESIZEEND
        ;}
        else if (Event = 0x800B) { ;; EVENT_OBJECT_LOCATIONCHANGE
            ;msgBox EVENT_OBJECT_LOCATIONCHANGE
        }
        ; Sleep, % cfg.onMessageDelay.shellEvent

        this.setWindowFloating(wnd, wnd.minMax != 0)
    }
    ...
    startEventListeners(){
        ...
        this.HookProcAdr := RegisterCallback(GeneralManager._onWindowEvent , "F", , &this)
        ;; listen for minimize / restore window events
        DllCall("CoInitialize", "uint", 0)
        this.hooks["min"] := DllCall( "SetWinEventHook"
            , UInt,0x0016
            , UInt,0x0017
            , Ptr,0
        ...
        ;; listen for maximize window events
        DllCall("CoInitialize", "uint", 0)
        this.hooks["resize"] := DllCall( "SetWinEventHook"
            , UInt,0x800B
            , UInt,0x800B
            , Ptr,0
            , Ptr,this.HookProcAdr
